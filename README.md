...
### Halting
```jvs
halt() - halts the program until any key is clicked
```
### Terminal
```jvs
clear() - clears the current terminal
prompt(answer, Enter a number: ) - prompts the user "Enter a number: " then stores what they typed in answer
```
